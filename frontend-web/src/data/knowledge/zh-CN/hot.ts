...
const hot: KnowledgeMap = {
  'edu.hot.language-advisor': {
    concept:
      '智能合约语言择优是整条热点学习路径的入口 Lab：它把用户对业务场景的自然语言描述，映射到 `language-choice-rules.yaml` 中的关键词规则，推荐最优合约语言与工具链组（EVM / Solana / Move / ZK / BTC 等）。它不写业务逻辑，而是教「先按场景选对语言与栈，再进入专题 Lab」。推荐结果里附带 suggested_lab，可一键跳到对应热点实验。',
    howItWorks: [
      '用户在 9 个场景 chip 中选择（如 ZK Rollup、Solana 高性能），或自定义 prompt 文本。',
      '前端以 scenario（prompt 文本）与 tags（场景 id）为 params 调用 simulate，规则引擎按关键词命中 language-choice-rules.yaml。',
      '返回 evaluation：recommended_language、toolchain_group、recommended_template、suggested_lab 与 audit_hints。',
      '若推荐含 suggested_lab，UI 显示「打开专题 Lab」按钮跳转；可选点击「提交多语言编译 Job」，以 taskType=HOT_MULTI_LANG_COMPILE 触发编译仿真。',
    ],
    params: [
      { name: 'scenario', desc: '业务场景描述文本（取自所选 chip 的 prompt 或自定义输入），是关键词匹配的主输入。' },
      { name: 'tags', desc: '当前所选场景 id（如 zk / solana / nft），用于辅助分类与命中规则。' },
      { name: 'language', desc: '编译 Job 阶段使用：从推荐结果带入的目标语言（如 cairo / solidity）。' },
      { name: 'allowed_chain_ids', desc: '允许的链 ID 白名单，固定为 Sepolia 测试网 (11155111)。' },
    ],
    evalFields: [
      { name: 'recommended_language', desc: '规则推荐的最优合约语言，如 solidity / cairo / rust / move。' },
      { name: 'toolchain_group', desc: '工具链组别（EVM / Solana / Move / ZK 等），决定后续编译镜像。' },
      { name: 'suggested_lab', desc: '建议下一步进入的专题 Lab 的 plugin id，用于路径串联。' },
      { name: 'compliance_passed', desc: '合规校验是否通过；通过后才展示推荐卡片。' },
    ],
    compliance: {
      allowed: ['测试网部署演示', '语言/工具链教学推荐', '多语言编译 Job 仿真'],
      forbidden: ['主网上线', '真实交易策略', '生产级 Rollup 部署'],
    },
    realWorld:
      '对应真实工程的技术选型阶段：架构师按业务约束（性能、安全、生态）在 Solidity / Rust / Move / Cairo 间选择语言与工具链，再搭建脚手架。',
    furtherReading: [
